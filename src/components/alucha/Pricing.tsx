    ...
    <section id="pricing" className="relative py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="max-w-2xl mb-12">
          <span className="text-xs uppercase tracking-[0.25em] text-alucha">{t("pricing.subtitle")}</span>
          <h2 className="mt-4 text-4xl sm:text-5xl font-bold">
            {t("pricing.titlePart1")}<span className="text-gradient-alucha">{t("pricing.titlePart2")}</span>
          </h2>
            ...
            <div className="space-y-8">
              <div>
                <div className="flex items-baseline justify-between mb-3">
                  <label className="text-sm font-medium">{t("pricing.pagesLabel")}</label>
                ...
                <div className="flex justify-between text-[11px] text-muted-foreground mt-2">
                  <span>{t("pricing.landing")}</span><span>{t("pricing.midsite")}</span><span>{t("pricing.platform")}</span>
                </div>
              </div>

              <div className="grid sm:grid-cols-3 gap-3">
                <Toggle label={t("pricing.ecom")} sub={t("pricing.ecomSub")} on={ecom} onClick={() => setEcom(!ecom)} />
                <Toggle label={t("pricing.3d")} sub={t("pricing.3dSub")} on={threeD} onClick={() => setThreeD(!threeD)} />
                <Toggle label={t("pricing.cms")} sub={t("pricing.cmsSub")} on={cms} onClick={() => setCms(!cms)} />
              </div>
            </div>
          </div>
          ...
          <aside className="relative glass-strong rounded-2xl p-8 overflow-hidden">
            ...
            <div className="relative">
              ...
              <div className="mt-2 text-sm text-muted-foreground">
                {t("pricing.weeksToLaunch").split("{weeks}").map((part, index) => (
                  <span key={index}>
                    {part}
                    {index === 0 && <span className="text-foreground font-medium">{weeks}</span>}
                  </span>
                ))}
              </div>
              <ul className="mt-6 space-y-2.5 text-sm">
                {[t("pricing.sprint1"), t("pricing.sprint2"), t("pricing.sprint3"), t("pricing.sprint4")].map((t) => (
