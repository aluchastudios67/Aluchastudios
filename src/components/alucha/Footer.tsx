    ...
    <footer className="relative border-t border-white/5 mt-20">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-14">
        <div className="grid md:grid-cols-[1.4fr_1fr_1fr_1fr] gap-10">
          <div>
            <a href="#top" className="flex items-center gap-3">
              ...
              <div className="leading-none">
                <div className="font-display font-semibold">{t("brand.fullname")}</div>
                <div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">{t("footer.address")}</div>
              </div>
            </a>
          ...
          <FooterCol title={t("footer.colStudio")} links={[t("footer.about"), t("footer.process"), t("footer.careers")]} />
          <FooterCol title={t("footer.colWork")} links={[t("footer.portfolio"), t("footer.pricing"), t("footer.caseStudies")]} />
          <FooterCol title={t("footer.colContact")} links={["hello@alucha.studio", "+995 555 00 00", t("footer.address")]} />
        </div>

        <div className="mt-12 pt-6 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-muted-foreground">© {new Date().getFullYear()} {t("brand.fullname")}. {t("footer.rights")}</p>
