  ...
  return (
    <section id="builder" className="relative py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 grid lg:grid-cols-[1fr_1.2fr] gap-10 items-center">
        <div>
          <span className="text-xs uppercase tracking-[0.25em] text-alucha">{t("builder.subtitle")}</span>
          <h2 className="mt-4 text-4xl sm:text-5xl font-bold leading-tight">
            {t("builder.titlePart1")}<span className="text-gradient-alucha">{t("builder.titlePart2")}</span>
          </h2>
          ...
          <div className="mt-8 space-y-5">
            <ToggleRow label={t("builder.controlTheme")}>
              <PillButton active={theme === "dark"} onClick={() => setTheme("dark")} icon={<Moon size={14} />}>{t("builder.themeDark")}</PillButton>
              <PillButton active={theme === "light"} onClick={() => setTheme("light")} icon={<Sun size={14} />}>{t("builder.themeLight")}</PillButton>
            </ToggleRow>
            <ToggleRow label={t("builder.controlLayout")}>
              <PillButton active={layout === "grid"} onClick={() => setLayout("grid")} icon={<Grid3x3 size={14} />}>{t("builder.layoutGrid")}</PillButton>
              <PillButton active={layout === "list"} onClick={() => setLayout("list")} icon={<List size={14} />}>{t("builder.layoutList")}</PillButton>
            </ToggleRow>
            <ToggleRow label={t("builder.controlAccent")}>
              <PillButton active={accent === "alucha"} onClick={() => setAccent("alucha")}>
                <span className="w-3 h-3 rounded-full" style={{ background: "var(--alucha)" }} /> {t("builder.accentAlucha")}
              </PillButton>
              <PillButton active={accent === "plum"} onClick={() => setAccent("plum")}>
                <span className="w-3 h-3 rounded-full" style={{ background: "var(--plum)" }} /> {t("builder.accentPlum")}
              </PillButton>
            </ToggleRow>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <div className="relative rounded-[2rem] p-3 glass-strong shadow-glow">
            ...
            >
              ...
              <p className="text-xs mt-2 opacity-70">{t("builder.mockDesc")}</p>
